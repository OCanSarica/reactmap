...
namespace dynamic_linq
{
    public static class DynamicQueryable
    {
        ...
        public static IQueryable Select(this IQueryable source, Type resultType, string selector, params object[] values)
        {
            if (source == null) throw new ArgumentNullException("source");
            if (selector == null) throw new ArgumentNullException("selector");
            LambdaExpression lambda = DynamicExpression.ParseLambda(source.ElementType, resultType, selector, values);
            return source.Provider.CreateQuery(
                Expression.Call(
        ...
        public static IQueryable Join(this IQueryable outer, IEnumerable inner, string outerKey, string outerSelector, string innerKey, string innerSelector, string resultsSelector, params object[] values)
        {
            ...
            ParameterExpression[] parameters = new ParameterExpression[] {
            Expression.Parameter(outer.ElementType, outerKey), Expression.Parameter(inner.AsQueryable().ElementType, innerKey) };
            LambdaExpression resultsSelectorLambda = DynamicExpression.ParseLambda(parameters, null, resultsSelector, values);

            return outer.Provider.CreateQuery(
                Expression.Call(
                    FindMethod(typeof(Queryable), "Join", true, 5).MakeGenericMethod(
        ...
        public static IQueryable<T> Join<T>(this IQueryable<T> outer, IEnumerable<T> inner, string outerKey, string outerSelector, string innerKey, string innerSelector, string resultsSelector, params object[] values)
        {
            return (IQueryable<T>)Join((IQueryable)outer, (IEnumerable)inner, outerKey, outerSelector, innerKey, innerSelector, resultsSelector, values);
        }

        private static MethodInfo FindMethod(Type type, string method)
        {
            return FindMethod(type, method, null, null, null, new Type[] { });
        }

        private static MethodInfo FindMethod(Type type, string method, bool? isGenericMethod)
        {
            return FindMethod(type, method, isGenericMethod, null, null, new Type[] { });
        }

        private static MethodInfo FindMethod(Type type, string method, bool? isGenericMethod, int? parameterCount)
        {
            return FindMethod(type, method, isGenericMethod, parameterCount, null, new Type[] { });
        }

        private static MethodInfo FindMethod(Type type, string method, bool? isGenericMethod, int? parameterCount, params Type[] parameterTypes)
        {
            return FindMethod(type, method, isGenericMethod, parameterCount, null, parameterTypes);
        }

        private static MethodInfo FindMethod(Type type, string method, bool? isGenericMethod, int? parameterCount, BindingFlags? bindingsFlag, params Type[] parameterTypes)
        {
            var methodInfoList = (bindingsFlag == null ? type.GetMethods() : type.GetMethods(bindingsFlag.Value))
                .Where(x => x.Name == method &&
                        (isGenericMethod != null ? x.IsGenericMethod == isGenericMethod : true) &&
                        (parameterCount != null ? x.GetParameters().Count() == parameterCount : true))
                .OrderBy(x => x.GetParameters().Count());

            if (parameterTypes != null && parameterTypes.Length != 0)
            {
                foreach (var info in methodInfoList)
                {
                    var parameterInfoList = info.GetParameters();

                    if (parameterInfoList.Length >= parameterTypes.Length)
                    {
                        int i = 0;
                        for (; i < parameterTypes.Length; i++)
                        {
                            ...
                            if (parameterInfoList[i].ParameterType != param)
                            {
                                break;
                            }
                        }
...
#pragma warning restore 0219
        ...
        Expression ParseNew()
        {
            ...
            while (true)
            {
                ...
                properties.Add(new DynamicProperty(propName, (newResultType != null ? newResultType.GetProperty(propName).PropertyType : expr.Type)));
                if (token.id != TokenId.Comma) break;
                NextToken();
            }
            ...
            List<MemberBinding> bindings = new List<MemberBinding>(properties.Count);
            for (int i = 0; i < properties.Count; i++)
            {
                var typeProp = type.GetProperty(properties[i].Name);
                if (typeProp.PropertyType == expressions[i].Type)
                    bindings.Add(Expression.Bind(typeProp, expressions[i]));
            }
